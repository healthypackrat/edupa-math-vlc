...
class Scraper
  ...
  def initialize(courses_path:, durations_path:, logger: Logger.new($stderr), wait_time: 1.5)
  ...
  def fetch_video_duration(video)
    ...
    if duration
      ...
      info = IO.popen(['ffmpeg', '-i', url, :err => [:child, :out]], &:read)
